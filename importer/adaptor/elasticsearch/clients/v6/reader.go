...
func init() {
	constraint, _ := version.NewConstraint(">= 6.0, < 7.0")
	clients.AddReader("v6", constraint, func(opts *clients.ClientOptions) (client.Reader, error) {
		...
		r.esClient = esClient
		r.logger = log.With("reader", "elasticsearch").With("version", 6)
		r.index = opts.Index
		// check appbase
		uri := opts.URLs[0]
...
func (r *Reader) listMappings(filterFn func(name string) bool) <-chan string {
	...
	go func() {
		defer close(out)
		res, err := r.esClient.PerformRequest(context.Background(), elastic.PerformRequestOptions{
			Method: http.MethodGet,
			Path:   fmt.Sprintf("/%s/_mapping", r.index),
		})
		if err != nil {
	...
	go func() {
		...
		for {
			...
			case t, ok := <-in:
				...
				searchService, _ := r.esClient.Search(r.index).Type(t).Size(chunkSize).Query(elastic.NewMatchAllQuery()).Sort("_id", true).Do(context.Background())
				...
				if hitsSize != 0 {
					...
					for searchService.TotalHits() >= currHits {
						...
						searchService, _ = r.esClient.Search(r.index).Type(t).Size(chunkSize).Query(elastic.NewMatchAllQuery()).SearchAfter(lastData.Id).Sort("_id", true).Do(context.Background())
						hitsSize = len(searchService.Hits.Hits)
						if hitsSize == 0 {
							break
						}
